...
@Component({
  selector: 'app-add-lc-tour-package',
  templateUrl: './add-lc-tour-package.component.html',
  styleUrls: ['./add-lc-tour-package.component.scss']
})
export class AddLCTourPackageComponent implements OnInit {
  ...
  constructor(private _formBuilder: FormBuilder,
    ...
    private main: LCTourPackagesMainComponent,
    ...
    this.addPackage(this.packageForm.value);
    }

    addPackage(ticket_type: LCTourPackage) {
      ticket_type.amount = this.packageForm.get('amount').value;
      ticket_type.net_Amount= this.packageForm.get('net_Amount').value
     // console.log(ticket_type)
    this._service.savePackage(ticket_type).pipe(first())
    .subscribe({
    next: (response) => {
      this.resetForm();
      this.packageDialog = false;
